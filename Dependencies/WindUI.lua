...
function aa.DiscordInvite(aq, ar, at)
    -- ar: Discord settings table
    -- at: Folder name for persistence
    
    if not ar.Enabled then return false end
    
    local au = "WindUI/" .. at .. "/Discord Invites"
    local av = ar.Invite
    local aw = ".txt" -- Configuration extension
    
    -- Create folder if it doesn't exist
    if isfolder and not isfolder(au) then
        makefolder(au)
    end
    
    -- Check if user already joined (if RememberJoins is enabled)
    if ar.RememberJoins and isfile and isfile(au .. "/" .. av .. aw) then
        return true -- Already joined, don't prompt again
    end
    ...
    if requestFunc then
        ...
        if not ay then
            warn("[WindUI] Discord invite failed: " .. tostring(az))
        end
    end
    
    -- Remember the invite if successful and RememberJoins is enabled
    if ax and ar.RememberJoins and writefile then
        pcall(function()
            writefile(au .. "/" .. av .. aw, "WindUI RememberJoins is true for this invite")
        end)
    end
